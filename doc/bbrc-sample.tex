...
\begin{algorithm}[t]
  ...
  \Else
    ...
    \For{$subgraph \in keys(hash)$}
      ...
      \If{$(p \gets SignificanceTest(hash[subgraph]), method) < alpha$}
...
\begin{algorithm}[t]
  ...
  \For{$i:=1 \to 50$}
    ...
    \State $\left[ \mathbf{subgraphs}, \mathbf{res^B} \right] \gets Algorithm\,\ref{alg:bbrc-sample}(trainSet,numBoots,alpha=0.05)$ \Comment{$numBoots=1$ for BBRC}
